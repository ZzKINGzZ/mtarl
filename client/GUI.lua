...
function PlayerGUI()
	...
	--Playermenu
	g_Playermenugui["Window1"] = guiCreateWindow(0.3, 0.25, 0.5, 0.5, "Spielermenü", true)
	...
	g_Playermenugui["Checkbox_freizeit"] = guiCreateCheckBox(0.05, 0.4, 0.2153, 0.0662, "Freizeit-Aktivitäten", false, true, tab5)
	g_Playermenugui["Checkbox_auto"] = guiCreateCheckBox(0.05, 0.5, 0.2153, 0.0662, "Autohäuser", false, true, tab5)
	g_Playermenugui["Checkbox_kranken"] = guiCreateCheckBox(0.05, 0.6, 0.2153, 0.0662, "Krankenhäuser", false, true, tab5)
	...
	g_Playermenugui["HelpMemo"] = guiCreateMemo(0.4, 0.05, 0.55, 0.9, "Wähle ein Hilfethema aus.", true, tab4)
	...
	--Spielmenü
	--[[g_spielmenu["window"] = guiCreateWindow(0.2318,0.2361,0.5807,0.5914,"Spielmenü",true)
...
function initalizePostGUI()
	if not g_guiloaded["postgui"] then
		...
		g_postgui["abort"] = guiCreateButton(0.3503,0.9156,0.2576,0.0638,"Schlißen",true,g_postgui["window"])
...
function initalizePostWriteGUI()
	if not g_guiloaded["postwritegui"] then
		...
		guiCreateLabel(0.0156,0.1399,0.0939,0.0376,"Epfänger:",true,g_postwritegui["window"])
...
function initalizeHouseBuyGUI()
	if not g_guiloaded["housebuygui"] then
		...
		guiComboBoxAddItem(g_housebuygui["combo"], "Möbel")
		...
		g_housebuygui["select"] = guiCreateButton(0.0134,0.8377,0.2405,0.0593,"Auswählen",true,g_housebuygui["window"])
...
function initalizeAutokaufGUI()
	if not g_guiloaded["autokaufgui"] then
		...
		g_carsale["ok"] = guiCreateButton(0.0461,0.885,0.4366,0.0877,"Bestätigen",true,g_carsale["window"])
		...
		g_carsale["price_memo"] = guiCreateMemo(0.3953,0.1002,0.5793,0.7825,"Bitte wähle ein Auto aus!",true,g_carsale["window_price"])
		...
		g_carsale["price_buy"] = guiCreateButton(0.1298,0.9041,0.3679,0.0725,"Kaufen",true,g_carsale["window_price"])
     	g_carsale["price_close"] = guiCreateButton(0.5298,0.9041,0.3679,0.0725,"Schließen",true,g_carsale["window_price"])
...
function initalizeTuningGUI()
	if not g_guiloaded["tuninggui"] then
		...
		--Steuerung
		g_tuning["window_strg"] = guiCreateWindow(0.05,0 ,0.8844,0.074,"Tunings Menü",true)
		...
		g_tuning["label_gruen"] = guiCreateLabel(0.0854,0.5072,0.0826,0.0718,"Grün:",true,g_tuning["window_licht"])
...
function initalizeWeaponLizenzGUI()
	if not g_guiloaded["weaponlizensgui"] then
		...
		g_lizenz["tip"] = guiCreateLabel(0.1105,0.1283,0.7532,0.0708,"Wählen Sie ein Waffen Klasse aus deren Lizenz Sie erwerben möchten!",true,g_lizenz["window"])
...
function initalizeFlightLizenzGUI()
	if not g_guiloaded["flightlizensgui"] then		
		...
		g_flightlizenz["tip"] = guiCreateLabel(0.0331,0.1174,0.9337,0.2435,"Wenn Sie eine Fluglizens erwerben möchten bestätigen Sie bitte mit \"Kaufen\" wenn nich brechen Sie den Vorgang ab!                                                                                     Eine Lizens Kostet 2000$.",true,g_flightlizenz["window"])
...
function initalizePWchangeGUI()
	if not g_guiloaded["pwchangegui"] then		
		g_pwchange["window"] = guiCreateWindow(0.4078,0.3375,0.1891,0.2625,"Passwort ändern",true)
		...
		g_pwchange["button_ok"] = guiCreateButton(0.1157,0.8056,0.5537,0.131, "Ändern",true,g_pwchange["window"])
...
function initalizeMailChangeGUI()
	if not g_guiloaded["mailchangegui"] then		
		g_mailchange["window"] = guiCreateWindow(0.4078,0.3375,0.1891,0.2625,"E-mail ändern",true)
		...
		g_mailchange["button_ok"] = guiCreateButton(0.1157,0.8056,0.5537,0.131, "Ändern",true,g_mailchange["window"])
...
function initalizeWahlGUI()
	if not g_guiloaded["wahlgui"] then
		...
		g_wahlengui["wahlen"] = guiCreateButton(0.4, 0.6, 0.2, 0.3, "Wählen!", true, g_wahlengui["window"])
...
function initalizeCreateInteriorGUI()
	if not g_guiloaded["cinterior"] then	
		...
		g_cinterior["radio_3"] = guiCreateRadioButton(0.4587,0.5508,0.1286,0.0763,"Groß",true,g_cinterior["window"])
		g_cinterior["radio_4"] = guiCreateRadioButton(0.6456,0.5508,0.2015,0.0805,"Extra Groß",true,g_cinterior["window"])
...
function initalizeJobEditGUI()
	if not g_guiloaded["jobeditgui"] then
		...
		g_jobeditgui["accept"] = guiCreateButton(0.0333, 0.8819, 0.9273, 0.0433, "Durchführen", true, g_jobeditgui["window"])
...
function initalizeGeldautomatGUI()
	if not g_guiloaded["geldautomatgui"] then
		...
		g_Moneymenu["Error"] = guiCreateLabel(0.0252,0.8938,0.5069,0.0973,"Fehler: Ungültiger Betrag!",true,tab1)
			guiLabelSetColor(g_Moneymenu["Error"],250,10,10)
		local tab2 = guiCreateTab("Überweisung",tabPanel)
		...
		g_Moneymenu["send"] = guiCreateButton(0.3830,0.7788,0.2431,0.1416,"".."Überweisen",true,tab2)
			guiSetProperty(g_Moneymenu["send"],"NormalTextColour","FFFF8C00")
			guiSetProperty(g_Moneymenu["send"],"HoverTextColour","FFFF4500")
		g_Moneymenu["ErrorSend"] = guiCreateLabel(0.0252,0.8938,0.5069,0.0973,"Fehler: Ungültiger Betrag!",true,tab2)
...
function initalizeStadtVerwaltungsGUI()
	if not g_guiloaded["stadtgui"] then
		...
		g_stadtgui["do"] = guiCreateButton(0.0353,0.5661,0.9216,0.1446,"Durchführen",true,g_stadtgui["window"])
...
function initalizeItemShopGUI()
	if not g_guiloaded["itemshop"] then
		...
		g_itemshopgui["button_close"] = guiCreateButton(0.5258,0.9239,0.2113,0.0543,"Schließen",true,g_itemshopgui["window"])
...
function initalizePlayerstatsGUI()
	if not g_guiloaded["playerstats"] then
		...
		g_playerstats["tabbedurefnisse"] = guiCreateTab("Bedürfnisse",g_playerstats["tabpanel"])
...
function initBugReportGUI()
	if not g_guiloaded["buggui"] then
		...
		g_bugGUI["schliessen"] = guiCreateButton(0.628,0.9089,0.3426,0.0694,"Schließen",true,g_bugGUI["window"])
...
function initRemoveItemGUI()
	if not g_guiloaded["removeitemgui"] then
		...
		g_removeItemGUI["do"] = guiCreateButton(0.0321,0.7487,0.4247,0.1658,"Zerstören",true,g_removeItemGUI["window"])
